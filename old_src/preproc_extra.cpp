/* ========================================================================
   $File: preproc_extra.cpp $
   $Date: January 25 2026 06:05 pm $
   $Revision: $
...
internal_api void
append_struct_member_to_definition(preprocessor_token_t member_type, string_builder_t *local_info_builder, preprocessor_token_t member_name)
{
    char buffer[256];
    ...
    c_string_builder_append_data(local_info_builder, struct_info_string);
}

internal_api void 
consume_until_semicolon(string_t *tokenized_data) 
{
    for(;;) 
    {
        preprocessor_token_t t = get_next_token(tokenized_data);
        if(t.type == TT_Semicolon || t.type == TT_EOF) break;
    }
}

// NOTE(Sleepster): Have member return whether or not this was an anonymous structure or something becuase it's counting nameless 
//                  items as members which is not correct. Maybe add back the closing bracket at the end of the structure function too

internal_api void 
parse_struct_member(string_t            *tokenized_data, 
                    bool8                is_pointer, 
                    string_builder_t    *local_struct_const_definition_builder, 
                    string_builder_t    *local_info_builder, 
                    preprocessor_token_t struct_name_token, 
                    preprocessor_token_t type_token)
{
    ...
    switch(token.type)
    {
        case TT_Asterisk: 
        {
            ...
            parse_struct_member(tokenized_data, is_pointer, local_struct_const_definition_builder, local_info_builder, struct_name_token, last_token);
        }break;
        case TT_Identifier:
        {
            ...
            {
                type_token = get_next_token(tokenized_data);
                if(type_token.type == TT_Identifier)
                {
                    if(append_type_enum_token(type_token))
                    {
                        return;
                    }
                }
            ...
            append_struct_member_to_definition(type_token, local_info_builder, token);
            ...
            c_string_builder_append_data(local_struct_const_definition_builder, member_data);
            is_pointer = false;
        }break;
        case TT_EOF:
        {
...
{   
    ...
    c_string_builder_init(&local_info_builder,                    MB(100));
    c_string_builder_init(&local_struct_const_definition_builder, MB(100));
    ...
    if(token.type == TT_OpeningBrace)
    {
        ...
        c_string_builder_append_data(&local_struct_const_definition_builder, struct_const_definition_string);
        ZeroMemory(buffer, sizeof(buffer));
        ...
        for(;;)
        {
            ...
            if(token.type == TT_ClosingBrace)
            {
                ...
                c_string_builder_append_data(&local_info_builder, STR("\t};\n\tu32 member_count;\n};\n\n"));
                break;
            }
            ...
            {
                parse_struct_member(tokenized_data, false, &local_struct_const_definition_builder, &local_info_builder, struct_name_token, token);
                ++member_count;
            }
        }
    ...
    string_t local_struct_const_definition_builder_string = c_string_builder_get_current_string(&local_struct_const_definition_builder);
    string_t local_info_builder_string                    = c_string_builder_get_current_string(&local_info_builder);

    c_string_builder_append_data(&state.struct_const_definition_builder, local_struct_const_definition_builder_string);
    c_string_builder_append_data(&state.struct_info_builder, local_info_builder_string);

    c_string_builder_deinit(&local_info_builder);
    c_string_builder_deinit(&local_struct_const_definition_builder);
        ...
        switch(token.type)
        {
            ...
            case TT_Identifier:
            {
                   ...
                   c_string_compare(token.string, STR("union")))
                {
                    ...
                    u32 member_count = parse_structure(&file_data, token);
                    (void)member_count;
