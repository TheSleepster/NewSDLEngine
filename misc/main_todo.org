...
** IMMEDIATE TODO
- [ ] Thread safe memory arenas...
- [ ] Arena allocator should allow the user to set it's growing status. That way, if the user wants to use
      the allocator on other threads, then we can safely do so. Right now with the way the arenas grow, it
      is not safe to use these on multiple threads.
- [ ] For consistency reasons, move all enums to full capitalization...
- [X] Simple shader preprocessor. It won't do anything insane, all it will do is look for "#include <file>" directives and handle them, spitting the processed file into the build directory.
- [X] IMGUI interface...
- [ ] File watcher with asset hotreloading (using some sort of sys_create_process() to start the jfd_file_packer)
- [ ] Possibly seperate the "main" and the "entry" files so that they are ".so" and ".exe" files. Game DLL hot reloading.
- [ ] Separate debug macros for Vulkan Validation Layers
- [ ] Debug macros for toggling certain slowdown points such as Assert() and Expect()
- [ ] Makefile setting for these macros

- [ ] Bug inside the metaprogram... It seems to hate this:
```cpp
union clear_value_t
{
    struct clear_depth_stencil_t {
        float32 depth;
        u32     stencil;
    }clear_depth_stencil;
...
```
    Error reported is:
    ./meta/GENERATED_program_RTTI.h:7351:120: error: no member named 'clear_color' in 'clear_value_t::clear_color_t'
     7351 |         {.name = "clear_color", .type = TYPE_clear_color, .size = sizeof(decltype(GENERATED_DEFAULT_clear_value_t.clear_color.clear_color)), .struct_info = (type_info_struct_t*)&type_info_struct_clear_color_const_data},
          |                                                                                   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ^

*** IN THE FUTURE
- [ ] We need a proper timing API. Something that can just be attached to a program as a header or lib 
      and timing record information about the process and certain subblocks. 
- [ ] Memory Allocation tracker
- [ ] Complete malloc replacement (thread safe heap allocator), meant for use where c_za_alloc() is currently used.

- [X] Replace malloc inside of the Linux sys_allocate and sys_free functions. It sucks.
- [X] Proper test suite that allows us to actually get REAL benchmarking data
- [X] Slang shader reflector tool. Slang reflect sucks ass we're replacing it... We have to use a tool
      like Slang Reflect because we want to move from loading spv from the disk, to instead just loading 
      the raw .slang shaders into the asset file and compile it live. This is for reasons of compatibility and
      quicker hotreloading in the future.

** GPU Resource Manager
the goal of this item is very simple, just allow us to control and
monitor gpu data in a way that is managed rather than
spontaneous. A perfect example is buffer staging and image
updates. Image updates and buffer updating should be performed
right before calling the main backend render
functions. Simple. These are meant to make sure the data is
sorted, managed, and appropriately handled without direct control.

- [ ] Track and handle buffer data
- [ ] Track and handle image data
- [ ] Some entity to manage buffers, allowing us to control things like all data being staged before the frame.
- [ ] Better support for both CPU mapped buffers and GPU exclusive buffers.
- [ ] Image manager like idSoftware's in DOOM3 BFG
- [ ] Image manager should have the ability to provide "scratch images" like our "scratch buffer" approach
- [ ] Perhaps change how we're using single use scratch buffers all over the place. See if that's really bad.

* Next Milestone:
- [X] Vulkan GPU memory allocator
- [X] Clear the screen
- [X] Swapbuffers command
- [X] Handle window resizing
- [X] Shaders and descriptor pools
- [X] Allocating and handling descriptor sets
- [X] Create our own custom GPU image format. Something like idImage

- [X] We need to completely rework descriptors and how they function. Right now, everything in the shader
      revolves around the shader, which is not right. We need to have the shader / material_archetype own it's
      descriptor_set_layout and have some sort of global descriptor manager that controls and allocates us data
      for these descriptors in accordance to the set layout stored within the shader. 

- [X] We should migrate from whatever you want to call this asscan system to a "constant_buffer" system. This way,
      we can allow the user to just give us data into the constant buffer and we'll just update it as the time comes
      in the vulkan renderer.
- [X] Index buffer support
- [-] Expanded draw command capabilities (in the future, draw_indirect...)
- [X] Automatically bind the renderpasses "read attachments" as shader sampled textures
- [X] blit_image command
- [-] Compute shader support (in theory, but we can't test it...)
- [X] Shader pipeline cache for different GL_States
- [X] MORE CLEANUP (specifically with the vk_backend_buffer functionality... it's REALLY bad...)
- [X] CLEANUP TODOs

- [X] We must be able to stage data more effectively

** Materials
- [X] Instead of cacheing uniforms, we should just use "constant buffers"
- [X] Establish the concept of a material
- [X] Default texture should always be in slot 0, make our texture array in the shader 0 indexed (we went with -1)
- [X] Materials. Materials should store what shader / blending states are needed for a given set of objects.
      (shader, blend modes, depth state, textures, camera data, etc.)
- [X] Perhaps some kind of "constant_buffer" object that maps the name of a buffer / uniform object inside a shader
      to some CPU side data that we can access easily.
- [ ] A program to take whatever the material file looks like and transform it into a binary blob.
      This way, parsing it and loading it is WAY faster. The asset file packer can probably do this.
      We should also give the engine the ability to do this for the sake of asset hot reloading.
...
- [X] We have a bug where we are using the swapchain's image count for
 ...
 - [X] Refactor "r_vulkan_begin_frame" and "r_vulkan_end_frame"
...
- [X] Modify all asset deletion so that we only delete the asset at the END of the game frame]
- [X] Have the catalog of a specific asset type keep track of what assets are loaded. This way we can simply loop over all assets that are loaded
** Textures
- [ ] Renderer's create Texture function only takes in pixels. The rest of the data is stored on the texture_t object
- [X] Actually make the "generation_ID" mean something
- [ ] PNG Decoder
** Fonts
- [X] Create a temporary glyph storage method, that way we can store what glyphs need to be added over the frame and add them at all at once at the end of the frame
- [x] Really font rendering in general
** Sounds
- [ ] The ability to play sound period
- [ ] Vorbis OGG decoder
** Shaders
...
- [X] Find out why the hell it's taking so long to pack
...
* Metaprogram
- [X] Type database for more efficient storage of types accross the program
- [X] Some sort of get_type_info(&structure) function that makes the retrieval of type info better.
- [X] The ability to do the same for members of structs get_member_info(structure_info, string_t member_name)
- [X] The ability to do the same for enums
- [X] Type table to allow the user to query for the type_info via type name as a string.
- [X] Nested structure support
- [X] Ignore #if 0 blocks completely
- [X] A better way of parsing modifiers like "volatile", "const", "static", etc.
- [X] Support Enums as well.

  (This is done for enums)
- [-] The ability to get the parent structure from the name of a member. (this might be impossible.)
      This might be impossible simply do to the fact that we can have the same member in many different structs.
      We can have token.data, bitmap.data, texture.data, etc. So how can we know which type_info_struct_t we want???
* Core stuff
- [X] Input flag parser
- [X] Type safe hash tables
- [X] Allow us to set the custom allocator for the hash tables
- [X] The ability to call a function like 'c_string_builder_flush' which would write out all the string builder's current buffers, and reset it's internal state
- [X] Fix the c_string_read_* functions.
- [ ] Why are we using Malloc with our dynamic arrays
- [ ] Compile the core into a single library. c_data.lib or something. then have c_base.h import all the core headers
- [X] Convert Dynarray.cpp and Dynarray.h into an STB style library like the hash_tables
...
- [ ] Vulkan style memory allocation for functions that would allocate
      ...
      internal allocator.
* Multithreading
- [X] Maybe just rebuild the whole system... it seems that this model isn't actually any good anymore
- [X] Fix the thread/synchronization API. It is bad. Why do we do it that way.
- [X] A manner with which to run work in batches and get the results of that completed work back from the threadpool
- [X] sys_semaphore_signal (instead of release...)
- [X] Work stealing
- [X] Improvement to adding work_orders to the queue of each thread, currently the locking makes us slow
- [X] Sync API should have sys_mutex_lock() which will block. As well as sys_mutex_try_lock()
      which won't block. The seperation is valuable.
- [ ] We should add other debug values to the mutexes, semaphores, and threads.
      Items like DEBUG_name, thread_id, redudant lock counter, etc. would be very helpful.
...
* Archive
** This is just stuff I don't think I really need anymore, but don't want to delete them in case I DO actually want them and I was wrong
...
- [ ] (X Don't need this) Shaders should "cache" special uniforms like "camera uniforms" "texture array uniforms"
