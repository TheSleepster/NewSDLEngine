...
⌄ Kinda goes with the below here ⌄  
...
- [ ] Have the catalog of a specific asset type keep track of what assets are loaded. This way we can simply loop over all assets that are loaded
** Textures
...
- [ ] Convert Dynarray.cpp and Dynarray.h into an STB style library like the hash_tables
...
- [ ] Fix the thread/synchronization API. It is bad. Why do we do it that way.
- [ ] A manner with which to run work in batches and get the results of that completed work back from the threadpool
- [ ] sys_semaphore_signal (instead of release...)
- [ ] Work stealing
- [ ] Improvement to adding work_orders to the queue of each thread, currently the locking makes us slow
- [ ] Sync API should have sys_mutex_lock() which will block. As well as sys_mutex_try_lock()
