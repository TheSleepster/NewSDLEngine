...
constexpr u32 MAX_WORK_ORDERS  = 100000;
...
thread_do_next_work_order(threadpool_thread_t *thread)
{
    ...
    {
        // TODO(Sleepster): Steal some work 
        result = true;
    }
...
main(void)
{
    ...
    threadpool->threads_awake = 12;

    u32 total_work_orders = 0;
    ...
    {
        threadpool_thread_t *thread = threadpool->threads + thread_index;
        total_work_orders += thread->work_order_count;

        sys_semaphore_release(&thread->work_available.semaphore, 1);
    }

    log_info("Total work orders: '%d'...\n", total_work_orders);
