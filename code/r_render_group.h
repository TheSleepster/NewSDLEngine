...
// TODO(Sleepster):
// Now that we have materials, we want to store what goes in render_groups based on their material archetype.
// render_groups will base by archetype while render_geometry_batch_t will do the same thing, but for INSTANCES of this
// material.
...
struct draw_frame_t
{
    ...
    struct {
        ...
        asset_handle_t         *active_material;
...
true_inline void r_set_active_material_constant(render_state_t *render_state, string_t name, void *value, u64 value_size);
true_inline void r_set_active_render_material(render_state_t *render_state, asset_handle_t *handle);

true_inline void r_render_group_end(render_state_t *render_state);
true_inline void r_render_group_reset_draw_frame(render_state_t *render_state, asset_manager_t *asset_manager);

true_inline void r_push_texture(render_state_t *render_state, vec2_t position, vec2_t size, vec4_t color, float32 rotation, asset_handle_t *texture_handle);
true_inline void r_push_rect(render_state_t *render_state, vec2_t position, vec2_t size, vec4_t color, float32 rotation);
