...
{
    Assert(thread_scope_stack.current_stack_depth - 1 >= 0);
...
internal_api code_type_t* symbol_table_register_typename(string_t type_name, u64 alias_id = INVALID_ID);
...
#undef X
            ...
            if(type->flags & AST_TYPE_MODIFIER_FLAG_ARRAY)
            {
                ...
                AST_expression_value_t value = evaluate_expression_AST(expression);
...
main(int argc, char **argv)
{
        ...
        switch(token.token_type)
        {
            ...
            case TOKEN_TYPE_TYPEDEF:
            {
                ...
                if(peek_token.token_type == TOKEN_TYPE_IDENT || peek_token.token_type == TOKEN_TYPE_ASTERISK)
                {
                    ...
                    code_type_t *type_to_alias = symbol_table_search_for_code_type(peek_token.data);
                    Expect(type_to_alias, "Failed to find a type to alias, input type was: '%.*s'... this is not a valid type...\n", fprint_token(peek_token));

                    symbol_table_register_typename(next_token.data, type_to_alias->ID);
    ...
    for(;;)
    {
        ...
        switch(token.token_type)
        {
            ...
            {
                if(thread_scope_stack.current_stack_depth > 0)
                {
                    pop_scope_stack();
                }
            }break;
...
#undef X
    ...
    {
        printf("====================================================================\n");
        AST_node_t *lambda = g_symbol_table.lambdas[index]; 
        symbol_table_infer_type(lambda);

        printf("AST Node Type: '%s' is found:\n", print_AST_node_type(lambda->node_type));
        printf("Type Name: %.*s\n", fprint_string(lambda->identifier));

        AST_type_t *type = &lambda->type;
        printf("Type ID is: %lu\n", type->code_type->ID);
        printf("Type alias of: %lu\n", type->code_type->alias_of);
        printf("Metatype: '%s'\n", get_metatype_string(type->code_type->code_metatype));
...
#define X(lambda, string, value) if(type->flags & lambda) { printf("%s\n", string); }
        AST_TYPE_MODIFIER_FLAGS(X)
#undef X
        printf("====================================================================\n");
    }
