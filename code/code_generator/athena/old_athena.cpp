...
enum AST_type_flags_t
{
    ...
    AST_TYPE_FLAG_MACRO    = BIT(6),

    AST_NUMBER_FLAG_SIGNED = BIT(7),
    AST_NUMBER_FLAG_FLOAT  = BIT(8)
};
...
struct macro_data_t
{
    ...
    // NOTE(Sleepster): This stores the TOKENS of the expanded macro. Not a traditional AST
    AST_node_t *tokens;
...
parse_macro_information(tokenizer_t *tokenizer, string_t filename, token_data_t macro_name)
{
    ...
    macro.tokens = c_arena_push_struct(&permanent_arena, AST_node_t);

    macro.tokens->assigned_name = macro.macro_name;
    macro.tokens->filename      = tokenizer->filename;
    macro.tokens->line_number   = tokenizer->line_count;
...
#if 1
    macro.tokens = c_arena_push_struct(&permanent_arena, AST_node_t);
    macro.tokens->node_type     = AST_NODE_TYPE_IDENTIFIER;
    macro.tokens->assigned_name = macro.macro_name;
    while(expansion.data.count > 0)
    {
        token_data_t expansion_token = c_tokenizer_get_next_token(&expansion);
        AST_node_t *new_node = AST_get_next_sibling_node(&expansion, macro.tokens);
        if(expansion_token.type == TT_Identifier)
        {
            new_node->node_type = AST_NODE_TYPE_IDENTIFIER;
        }
        else if(expansion_token.type == TT_Number)
        {
            new_node->node_type = AST_NODE_TYPE_NUMBER;
        }
        else
        {
            // TODO(Sleepster): This might not be smart... but oh well 
            new_node->node_type = AST_NODE_TYPE_STRING_LITERAL;
        }
    ...
    while(expansion.data.count > 0)
    {
                                                     ...
                                                     peek_token.type == TT_OpeningParen))
        {
            ...
            type_data->literal_value = macro.expansion_string; 
            type_data->type_flags   |= AST_TYPE_FLAG_MACRO;
...
generate_typedef_AST(tokenizer_t *tokenizer, token_data_t token, AST_node_t *new_node)
{
    ...
    {
        // NOTE(Sleepster): If the end_peek_token is a ';', it's a Simple typedef, we don't even need an AST for these. Just register them immediately.
        token_data_t end_of_declaration = c_tokenizer_peek_token(tokenizer, 2);
        token_data_t complex_typedef    = c_tokenizer_peek_token(tokenizer, 1);
        if(end_of_declaration.type == TT_Semicolon)
        {
        ...
        else if(complex_typedef.type == TT_OpeningParen)
        {
            // NOTE(Sleepster): Do nothing. 
        }
        else
        {
            Expect(false, "The token: '%.*s' was unexpected inside our typedef logic, and therefore was not handled...\n",
                   fprint_token(type_token));
        }
    }
}
