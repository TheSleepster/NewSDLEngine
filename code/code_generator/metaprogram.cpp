                ...
                meta_struct_t *last_struct)
{
    ...
    while(parsing)
    {
        ...
        switch(token.type)
        {
            ...
            {
                meta_member_t member = {};
                token_data_t peek_token = c_tokenizer_peek_token(&file_data->tokenizer, 1);
...
parse_enum(ast_file_data_t *file_data, token_data_t structure_type)
{
    ...
    if(enum_type_name_token.type == TT_OpeningBrace)
    {
        return;
    }

    Expect(enum_type_name_token.type == TT_Identifier, "Token was not an TT_Identifier, instead was: '%.*s'\n",
           enum_type_name_token.string.count, C_STR(enum_type_name_token.string));
    ...
    switch(token.type)
    {
        case TT_HashTag:
        {
            ...
            if(c_string_compare(token.string, STR("if")))
            {
                ...
                else if(peek_token.string.data[0] == '1')
                {
                    token = c_tokenizer_get_next_token(&file->tokenizer);
                }
            }
            else if(c_string_compare(token.string, STR("else")))
            {
                bool8 parsing = true;
                while(parsing)
                {
...
#if 1 
    visit_file_data_t visit_info = c_directory_create_visit_data(generate_file_metadata, true, null);

    byte buffer[4096] = {};
    sys_directory_get_current_working_dir(buffer, sizeof(buffer));
    c_directory_visit(STR((const char*)buffer), &visit_info);
#else
